...
def y_ordinate_chooser(label, idn, colour_default, lm_default, marker_default, line_default,
                       size_default):
    return html.Div([
        html.Div(label,
                 style={'display': 'inline-block', 'vertical-align': 'middle', "width": right_div_label_width}),
...
[
    ...
    [
        ...
        [
            data_chooser("Data", 'data-chooser-dropdown'),
        ],
            style={'height': "10vh", 'vertical-align': 'middle'}),
        html.Div(children=  # plot controls
        [
            # ---
            x_ordinate_chooser("X axis", "x-chooser-dropdown"),
            y_ordinate_chooser("Y axis 1", "y-chooser-dropdown-1", dict(hex='#0000FF'), "markers", "cross", "solid", 6),
            y_ordinate_chooser("Y axis 2", "y-chooser-dropdown-2", dict(hex='#FF0000'), "lines", "circle", "solid", 6),
            y_ordinate_chooser("Y axis 3", "y-chooser-dropdown-3", dict(hex='#909090'), "lines", "circle", "solid", 6),
            y_ordinate_chooser("Y axis 4", "y-chooser-dropdown-4", dict(hex='#606060'), "lines", "circle", "solid", 4),
            # ---
            # ---
            html.P(),
            # ---
...
@app.callback(
    [
        Output('data-chooser-dropdown', "options"), Output('data-chooser-dropdown', "value"),
        Output("x-chooser-dropdown", "options"), Output("x-chooser-dropdown", "value"),
        Output("y-chooser-dropdown-1", "options"), Output("y-chooser-dropdown-1", "value"),
        Output("y-chooser-dropdown-2", "options"), Output("y-chooser-dropdown-2", "value"),
        Output("y-chooser-dropdown-3", "options"), Output("y-chooser-dropdown-3", "value"),
        Output("y-chooser-dropdown-4", "options"), Output("y-chooser-dropdown-4", "value"),
        Output("all-the-data", "data")
    ],
    [Input("file-upload", "contents")]
)
def update_dropdowns_and_store(contents):
    print("update_dropdowns_and_store")
    df = parse_uploaded_file(contents)
    patterns = df["_pd_block_id"].unique()
    data_list = [{"label": str(pattern), "value": str(pattern)} for pattern in patterns]

    # todo: make the values for the dropdowns sensitive to the actual y ordinates possible
    ...
    for pattern in patterns:
        all_patterns[pattern] = df[df["_pd_block_id"] == pattern].to_json(orient='split', date_format='iso')
    all_patterns["df"] = df.to_json(orient='split', date_format='iso')

    #idea: can I store each pattern (and also the entrie dataset) it it's own dcc.store? Might need
    # pattern matching callbacks in order to pull the correct data. That could ease the transfer of data.


    return (data_list, data_list[0]["value"],
            x_list, x_list[0]["value"],
            y_list, y_list[0]["value"],
            y_list, y_list[1]["value"],
            y_list, y_list[2]["value"],
            y_list, y_list[3]["value"],
            all_patterns
            )


@app.callback(Output("plot", component_property="figure"),
              [Input("all-the-data", component_property="data"),
...
def update_single(data, pattern, x_ordinate, y_ordinates, y_styles):
    #  if pattern is None:
    ...
    filtered_df = pd.read_json(data[pattern], orient="split")  # data[pattern] is JSON data.
    ...
    layout = go.Layout(title={"text": pattern, "x": 0.1},
